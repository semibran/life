...
var game = Game$$1.create(WORLD_SIZE, '#app');
var paused = false;

function animate() {

  if (keys.tapped.KeyR && !keys.pressed.ControlLeft) {
    if (!keys.pressed.ShiftLeft) game.world = World.generate(WORLD_SIZE);else game.world = new Uint8ClampedArray(WORLD_SIZE * WORLD_SIZE);
    game.display.render(game.world);
  }

  if (!keys.pressed.KeyP) {
    if (keys.tapped.Space) paused = true, document.body.classList.add('hide-cursor');else if (keys.released.Space) paused = false, document.body.classList.remove('hide-cursor');
  } else if (keys.tapped.KeyP) paused = !paused;

  if (!paused && !mouse.left && !mouse.right) game.tick();
...
function onKeyDown(event) {
  var code = event.code;
  if (!keys.pressed[code]) keys.tapped[code] = true;
  keys.pressed[code] = true;
}

function onKeyUp(event) {
  var code = event.code;
  if (keys.pressed[code]) keys.released[code] = true;
